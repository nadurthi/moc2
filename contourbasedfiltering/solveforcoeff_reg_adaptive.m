function [lam,err]=solveforcoeff_reg_adaptive(A,b,Aineq,bineq,Nlam2ndorder,maxrelerr_percent,method)
...
if strcmp(method,'LPguru')
    ...
    while true
        [Ca,Cb]
        if adone==0
            fa=[zeros(lamdim,1);Ca/lamdim*ones(lamdim,1);10];
            [xa,fvala,exitflaga,outputa,lambdaa] = linprog_guru(fa,AAineq,BBineq,[],[],lb,ub,options);
            lama = xa(1:lamdim);
            err=A*lama-b;
            errpera = 100*abs( err(1)/(b(1)) )
        end
        if bdone==0
            fb=[zeros(lamdim,1);Cb/lamdim*ones(lamdim,1);10];
            [xb,fvalb,exitflagb,outputb,lambdab] = linprog_guru(fb,AAineq,BBineq,[],[],lb,ub,options);
            lamb = xb(1:lamdim);
            err=A*lamb-b;
            errperb = 100*abs( err(1)/(b(1)) )
        end
        
        if errpera<=maxrelerr_percent && errperb>=maxrelerr_percent
            break;
        end
        if errpera>=maxrelerr_percent
            Ca=Ca/5;
        else
            adone=1;
        end
        if errperb<=maxrelerr_percent
            Cb=5*Cb;
        else
            bdone=1;
        end
    ...
    disp('Marching to get bounds done')
    while true
        if abs(Ca-Cb)<=100
            break
        end
        ...
        [Ca,Cb,C, 100*abs( err(1)/(b(1)) ),exitflag]
