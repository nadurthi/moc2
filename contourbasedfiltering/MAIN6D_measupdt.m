...
time.tf=2*48*60*60 *constants.trueT2normT;
...
model.pointGenerator = @(mx,Px)hybridpoints(mx,Px,3,0.3,5,0.8);
...
plotfolder='simulations/SAT6Dsim1_measupdt_smallR01';
mkdir(plotfolder)
...
%             [X,probs,fullnormpdf] =       MeasUpdt_character_modf(X,probs,3,k,zk,Xtruth(k,:),model,Xmctest,11);
            [X,probs,fullnormpdf] = MeasUpdt_character_modf_6Dsat(X,probs,priorfullnormpdf,4,k,zk,Xtruth(k,:),model,time,Xmctest,5);
