function pdfnorm = get_interp_pdf_0I_6D_gobackget_L1(model,priorfullnormpdf,Xk1,probsk1,Nm,Tstepk1,Tk1,dtkk1,Xmctest,Xtruth)
...
Xout = Xout(~indxout,:);
disp(['Number of out reg points are : ',num2str(size(Xout,1))])
...
disp(['got back probs : ',num2str(toc)])
...
disp(['got basis poly : ',num2str(toc)])
lamdim = length(Pf);
...
Aeqtrain = A(indeq,:);
Aineq = A(~indeq,:);
disp(['got all the As : ',num2str(toc)])
