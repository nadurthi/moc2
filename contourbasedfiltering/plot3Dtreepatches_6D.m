function plot3Dtreepatches_6D(boxes,states)
...
for r=1:size(boxes,1)
    ...
    patch(dr(:,states(1)),dr(:,states(2)),boxes{r,4}*ones(size(dr,1),1),'g','linewidth',1)
    plot(dr(:,states(1)),dr(:,states(2)),'k','linewidth',0.5)
    alpha(0.3)
