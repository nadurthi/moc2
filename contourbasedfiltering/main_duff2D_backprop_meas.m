...
plotfolder='simulations/duff2D_meas_bringback';
mkdir(plotfolder)
...
%     GMM=prior_prop_GMM(GMM,time.dt,time.Tvec(k),model);
    ...
    if k>=2
        if rem(k,meas_freq_steps)==0
            ...
            % %%%%%%%%%%%%%%%%% MEAS UPDATE %%%%%%%%%%%%%%%%%%%%%%
            [priorfullnormpdf,postfullnormpdf,X,probs] = get_interp_pdf_0I_2D_gobackget_measupt(zk,model,priorfullnormpdf,X,probs,4,k,time.Tvec(k),time.dt,Xmctest,Xtruth(k,:));
