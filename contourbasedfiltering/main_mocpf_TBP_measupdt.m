...
model.R=diag([(0.5*pi/180)^2,(0.5*pi/180)^2]);
...
[X,w] = mvnrnd(zeros(model.fn,1),0.5*eye(model.fn),200000);
% X=[X1;X2];
...
dsX.AddMeanCov_to_OI_Trasform(xf0(:),3^2*Pf0);
...
plotfolder='simulations/SAT6DCOE_measUPDATE_treefilter2';
mkdir(plotfolder)
...
%     Xmctest=[];
    ...
    [priorfullnormpdf,postfullnormpdf,Xnptk1_prior,probntk1_prior,Xnptk1_post,probntk1_post]=moc_pf_6D(postfullnormpdf,k-1,time,NaN,model,struct('genpriorpdf',1,'genpostpdf',0,'plotstates',[1,2]));
    keyboard
...
%             keyboard
    ...
    save('plot6DtreePDF')
    % plot PRIOR --------------
    plorprior_treepdf_6D(k,model,Xtruplot,Xtruth,priorfullnormpdf,Xmctest,Xnptk1_prior,probntk1_prior,X_pf,xfquad,Pfquad,savePriorProps)
%     
    % plot POST --------------
