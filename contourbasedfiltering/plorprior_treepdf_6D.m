function plorprior_treepdf_6D(k,model,Xtruplot,Xtruth,priorfullnormpdf,Xmctest,Xnptk1_prior,probntk1_prior,X_pf,xfquad,Pfquad,saveprops,states)
nametag='prior';
...
[xx,yy]=meshgrid([linspace(-2,-0.7,5),linspace(-0.68,0.68,55),linspace(0.7,2,5)]);
...
NN=1e3;
intrpoints = mvurnd(-1.5*ones(1,4),1.5*ones(1,4),NN);
for i=1:size(xx,1)
    for j=1:size(xx,2)
        for r=1:NN
            xp = [xx(i,j),yy(i,j),intrpoints(r,:)];
            ssp = priorfullnormpdf.transForms.normX2trueX(xp);
            xxtruth(i,j) = ssp(1);
            yytruth(i,j) = ssp(2);
            
            s = priorfullnormpdf.normeval(xp);
            pptruth(i,j) = pptruth(i,j)+priorfullnormpdf.transForms.normprob2trueprob(s);
            ppnorm(i,j) = ppnorm(i,j)+s;
            
            s = mvnpdf(xp,xfqnorm(:)',Pfqnorm);
            ppquadtruth(i,j) = ppquadtruth(i,j)+priorfullnormpdf.transForms.normprob2trueprob(s);
            ppquadnorm(i,j) = ppquadnorm(i,j)+s;
        end
    end
end
pptruth=pptruth/NN;
ppnorm=ppnorm/NN;
ppquadtruth=ppquadtruth/NN;
ppquadnorm=ppquadnorm/NN;
