function [Xpost,postprobs] = SimpleGHpostSampler_6D(Xprior,priorprobs,probsXpost,priorpdfnorm,model,z,N1d,factor)
...
[mXpost,PXpost] = MeanCov(Xprior,probsXpost/sum(probsXpost));

lambda =0.9;
mX = lambda*mXpost+(1-lambda)*mXprior;


[XpriorNew,~] = model.pointGenerator(mX,(1)^2*PXpost);
...
[mXpost,PXpost] = MeanCov(Xpost,postprobs/sum(postprobs));
Xpostnorm = zeros(size(Xpost));
sqrtPinv = sqrtm(inv(PXpost));
for i=1:size(Xpostnorm,1)
   Xpostnorm(i,:) =  sqrtPinv*(Xpost(i,:)'-mXpost);
end
ind = sqrt(sum(Xpostnorm.^2,2))>4;
Xpost(ind,:)=[];
postprobs(ind)=[];
...
figure(112)
plot3(Xprior(:,1),Xprior(:,2),priorprobs/sum(priorprobs),'ro',Xprior(:,1),Xprior(:,2),probsXpost/sum(probsXpost),'b+',XpriorNew(:,1),XpriorNew(:,2),priorprobsNew/sum(priorprobsNew),'gs',Xpost(:,1),Xpost(:,2),postprobs/sum(postprobs),'k^')
