function [priorpdfk1,postpdfk1,Xnptk1_prior,probntk1_prior,Xnptk1_post,probntk1_post]=moc_pf_4D(pdfk,tk,time,zk1,model,optns)
...
% dsXkminus1 = DataSet(pdfk.Xrep,pdfk.prep,'TrueState');
% [mXkminus1,PXkminus1]=MeanCov(pdfk.Xrep,pdfk.prep/sum(pdfk.prep));
% dsXkminus1.AddMeanCov_to_OI_Trasform(mXkminus1,4^2*PXkminus1);
...
% mXk1=Xk1(b,:)';
dsX.AddMeanCov_to_OI_Trasform(mXk1,4^2*PXk1);
% dsX.AddHyperCubeTrasform(-0.9*ones(dim,1),0.9*ones(dim,1));
Y = dsX.ApplyAffineTransform_Final2Original(dsX.X,ones(N,1));
Xrepk1(1:20,:)-Y(1:20,:)
...
%             XXnptk1 = mvurnd(1*LB,1*UB,dim*100);
            XXnptk1=[dsX.X;mvurnd_expanded(1*LB,1*UB,2000,0)];
%             probntk1=dsX.p
        else
...
%             
            ...
            nids = pmf_sampler(MM,dim*100);
            for i=1:nb
                ns = sum(nids==i);
                lb = boxes{i,1};
                ub = boxes{i,2};
                XXnptk1=[XXnptk1;mvurnd_expanded(lb,ub,20*ns,0)];
            end
            
            nids=pmf_sampler(Assratio,dim*100);
            for i=1:nb
                ns = sum(nids==i);
                lb = boxes{i,1};
                ub = boxes{i,2};
...
%         XXptk1(1:200,:)-Xrepk1(1:200,:)
        
        XXnptk=pdfk.transForms.trueX2normX(XXptk);
%         XXptk(1:200,:)-pdfk.Xrep(1:200,:)
        
        Pprobntk = pdfk.normeval(XXnptk);
        
        Pprobtk=pdfk.transForms.normprob2trueprob(Pprobntk);
        
        Qp = pdfk.transForms.trueprob2normprob(pdfk.prep);
        QX = pdfk.transForms.trueX2normX(pdfk.Xrep);
%         XXnptk(1:200,:)-QX(1:200,:)
%         Qp-pdfk.normeval(QX)
        
        
%         (QX>repmat(LB',size(QX,1),1))
        insideind = sum((XXnptk>pdfk.LB') & (XXnptk<pdfk.UB'),2)==dim;
        sum(insideind)
        
        figure(49)
        plot3(XXnptk(:,1),XXnptk(:,2),Pprobntk,'bs',QX(:,1),QX(:,2),Qp,'r*')
        legend('back prop - norm','Xrep-norm')
        title('pos k-1')
        axis([-2,2,-2,2])
        figure(50)
        plot3(XXnptk(:,3),XXnptk(:,4),Pprobntk,'bs',QX(:,3),QX(:,4),Qp,'r*')
        legend('back prop - norm','Xrep-norm')
        title('vel k-1')
        axis([-2,2,-2,2])
...
%         Xnptk1=[Xnptk1;XXnptk1(insideind,:)];
%         probntk1=[probntk1;Pprobntk1(insideind)];
        ...
        for jj=1:Ntree
            ...
            % Now build the tree
            tree = fitrtree(Xtrain,ytrain,'MinParentSize',5,'MaxNumSplits',50000,'MinLeafSize',5);
...
%         EE=sortrows(EE,3);
        ...
        plot3Dtreepatches_6D(boxes(1:50,:),optns.plotstates)
        hold off
        toc
...
%         Assratio = Assratio/sum(Assratio);
        ...
        inds = Assratio>s;
        Assratio(inds)=Assratio(inds)*25;
        if sum(Assratio)~=0
            Assratio = Assratio/sum(Assratio);
        end
        toc
